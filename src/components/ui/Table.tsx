            ...
            {data.length === 0 ? (
              ...
              data.map((item, index) => (
                <tr
                  ...
                  className={`group transition-all duration-200 hover:bg-purple-50/50 ${onRowClick ? 'cursor-pointer' : ''}`}
...
export function SimpleTable({ children, className = '' }: SimpleTableProps) {
  return (
    <div className={`overflow-hidden rounded-2xl border border-slate-200/50 bg-white shadow-sm ${className}`}>
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          {children}
        </table>
...
export function TableRow({ children, onClick, className = '' }: { children: React.ReactNode; onClick?: () => void; className?: string }) {
  return (
    <tr
      className={`group transition-all duration-200 hover:bg-purple-50/50 ${onClick ? 'cursor-pointer' : ''} ${className}`}
      onClick={onClick}
    >
      {children}
    </tr>
        ...
        <TableRow key={rowIndex}>
          {Array.from({ length: cols }).map((_, colIndex) => (
            <TableCell key={colIndex}>
              <div className="h-4 bg-slate-200 rounded-lg animate-pulse" style={{ width: `${60 + Math.random() * 40}%` }} />
            </TableCell>
          ))}
        </TableRow>
      ))}
